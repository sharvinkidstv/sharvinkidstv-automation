...
def background(draw, t, idx):
    ...
    for i in range(22):
        ...
        for a in (0, 90, 180, 270):
            ...
            dx = int(8 * math.cos(math.radians(a)))
            dy = int(8 * math.sin(math.radians(a)))
...
def main():
    ...
    script = (
        Path(sys.argv[1])
        if len(sys.argv) > 1
        else ROOT / "scripts" / "abc-song.json"
    )
    ...
    # Clean previous generated frames/clips.
    ...
    for p in OUTPUT.glob("clip_*.mp4"):
        p.unlink()

    for p in OUTPUT.glob("voice_*.wav"):
        p.unlink()

    for p in OUTPUT.glob("music*.wav"):
        p.unlink()
